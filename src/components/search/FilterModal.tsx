...
export default function FilterModal({
  ...
  const roomTypes = initialFilters.roomTypes || [];
  const equipment = initialFilters.equipment || [];
  ...
  return createPortal(
    <AnimatePresence>
      {isOpen && (
        <div key="filter-modal-portal" className="fixed inset-0 z-[99999] flex items-center justify-center p-4">
          {/* Backdrop */}
          <motion.div
          ...
          {/* Modal Card */}
          <motion.div
            key="filter-modal-card"
            initial={{ opacity: 0, scale: 0.95, y: 15 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            exit={{ opacity: 0, scale: 0.95, y: 15 }}
            transition={{ duration: 0.2, ease: "easeOut" }}
            className="relative w-full max-w-[560px] max-h-[90vh] bg-white dark:bg-[#121a30] text-slate-900 dark:text-white rounded-3xl p-6 shadow-2xl z-10 border border-gray-100 dark:border-slate-800 flex flex-col justify-between overflow-hidden transition-colors"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="flex items-center justify-between pb-3 border-b border-gray-100 dark:border-slate-800 shrink-0">
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">All Filters</h2>
              <button
                type="button"
                onClick={onClose}
                className="w-8 h-8 rounded-full bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 text-gray-500 dark:text-slate-400 flex items-center justify-center cursor-pointer border-none transition-colors"
              >
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    const isChecked = roomTypes.includes(t.id);
                    return (
                      <button
                        key={t.id}
                        type="button"
                        ...
                        className={`p-3 rounded-2xl border flex items-center gap-2.5 cursor-pointer transition-all text-left ${
                          isChecked
                            ...
                            : "bg-white dark:bg-[#0e162b] hover:bg-gray-50 dark:hover:bg-[#16203a] border-gray-200 dark:border-slate-800 text-gray-700 dark:text-slate-300"
                        }`}
                      >
                  ...
                  <div>
                    ...
                    <div className="relative flex items-center">
                      <input
                        ...
                        className="w-full h-10 pl-3 pr-8 rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-xs font-bold text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#4a77ec]"
                      />
              ...
              <div className="space-y-3 pt-3 border-t border-gray-100 dark:border-slate-800">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold text-gray-700 dark:text-slate-300">Bedrooms</span>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold text-gray-700 dark:text-slate-300">Beds</span>
                    <div className="flex items-center gap-1">
                      {[0, 1, 2, 3, 4, 5].map((v) => (
                        <button
                          key={v}
              ...
              <div className="space-y-3 pt-3 border-t border-gray-100 dark:border-slate-800">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    const isChecked = equipment.includes(amenity.id);
                    return (
                      <button
                        key={amenity.id}
                        type="button"
                        onClick={() => {
                          const next = isChecked
                            ? equipment.filter((x) => x !== amenity.id)
                            : [...equipment, amenity.id];
